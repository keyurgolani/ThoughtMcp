...
function SceneContent({
  ...
  // Track expanded node for node-to-panel transformation (Requirements: 26.1-26.6)
  const [expandedNodeId, setExpandedNodeId] = useState<string | null>(null);

  // Handle background click to collapse expanded panel (Requirement 26.4)
  const handleBackgroundClick = useCallback(() => {
    if (expandedNodeId !== null) {
      setExpandedNodeId(null);
    }
    onBackgroundClick?.();
  }, [expandedNodeId, onBackgroundClick]);
  ...
  const handleNodeClick = useCallback(
    (nodeId: string) => {
      // If clicking the same node that's expanded, collapse it
      if (expandedNodeId === nodeId) {
        setExpandedNodeId(null);
      } else {
        // Collapse any previously expanded node and expand the clicked one (Requirement 26.6)
        setExpandedNodeId(nodeId);
      }
      onNodeClick?.(nodeId);
    },
    [onNodeClick, expandedNodeId]
  );

  // Handle panel close - collapse the expanded node (Requirements: 26.4, 26.5)
  ...
  return (
    <>
      ...
      {neighbors.map((neighbor: NeighborNode) => {
        ...
        // Create a memory-like object for the neighbor node for panel display
        const neighborMemoryData = {
          id: neighbor.id,
          content: neighbor.content,
          createdAt: neighbor.createdAt,
          lastAccessed: neighbor.createdAt, // Use createdAt as fallback
          accessCount: 0,
