...
export interface NeuronNode3DProps extends MemoryNodeDisplayProps {
  ...
  reducedMotion?: boolean;
  /** Whether this node is expanded into a panel (Requirements: 26.1-26.6) */
  isExpanded?: boolean;
  ...
  lightMode?: boolean;
  /** Full memory data for expanded panel display */
  memoryData?: {
    id: string;
    content: string;
    createdAt: string;
    ...
    userId: string;
    sessionId: string;
    primarySector: 'episodic' | 'semantic' | 'procedural' | 'emotional' | 'reflective';
    metadata: {
      keywords?: string[];
      tags?: string[];
      category?: string;
  ...
  onPanelClose?: () => void;
  /** Callback when edit is requested from panel */
  onEdit?: () => void;
  /** Callback when a cognitive action is triggered from panel */
  onCognitiveAction?: (
    action:
      | 'analyze-reasoning'
      | 'detect-bias'
...
export function NeuronNode3D({
  ...
  // Track node visibility during expansion animation
  ...
  useFrame((state, delta) => {
    ...
    // Handle expansion animation - fade out node when expanding (Requirements: 26.1, 26.3)
    const targetOpacity = isExpanded ? 0 : 1;
    const newOpacity = THREE.MathUtils.lerp(nodeOpacity, targetOpacity, Math.min(1, delta * 5));
    setNodeOpacity(newOpacity);

    // Smooth scale interpolation (skip if expanded)
    if (!isExpanded) {
    ...
    // Pulsing animation for current node or highlighted node (skip if expanded)
    // Requirement 42.6: Highlighted nodes from tag hover get a distinctive pulse
    if ((isCurrent || isHighlighted) && !reducedMotion && !isExpanded) {
    ...
    if (!isCurrent && !reducedMotion && !isExpanded && !effectiveHovered) {
      groupRef.current.rotation.y += IDLE_ROTATION_SPEED * delta;
    }
  });

  // Create Memory object for ExpandedNodePanel if memoryData is provided
  const memoryForPanel: Memory | null = useMemo(() => {
    if (!memoryData) return null;
    return memoryData as Memory;
  }, [memoryData]);
  ...
  return (
    <group ref={groupRef} position={position} name={`neuron-node-${id}`}>
      {/* Node visuals - fade out when expanded (Requirements: 26.1, 26.3) */}
      {nodeOpacity > 0.01 && (
        <>
          {/* Outer glow layer */}
          <SomaGlow
          ...
          {/* Billboard text label - hide when expanded */}
          <NodeLabel
            content={contentPreview || content}
            size={baseSize}
            visible={showLabel && !isExpanded}
          />
      ...
      {/* Expanded panel - shows when node is expanded (Requirements: 26.1, 26.2, 26.3, 40.2, 40.3) */}
      {memoryForPanel && (
        <ExpandedNodePanel
          memory={memoryForPanel}
          position={[0, 0, 0]}
          isExpanded={isExpanded}
          onClose={onPanelClose ?? ((): void => {})}
          onEdit={onEdit}
          onCognitiveAction={onCognitiveAction}
