...
/**
 * Extended result that includes both synthesized result and individual stream results
 */
export interface ExtendedSynthesizedResult extends SynthesizedResult {
  /** Individual stream results for detailed reporting */
...
export class ParallelReasoningOrchestrator {
   ...
   * @returns Promise resolving to extended synthesized result with individual stream results
   */
  async executeStreams(
    problem: Problem,
    streams: ReasoningStream[],
    totalTimeout: number = this.defaultTotalTimeout
  ): Promise<ExtendedSynthesizedResult> {
    ...
    // Return extended result with individual stream results
    return {
      ...synthesizedResult,
