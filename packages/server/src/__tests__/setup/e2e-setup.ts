...
function setPostgresEnvironment(): void {
  process.env.DATABASE_URL =
    process.env.DATABASE_URL ??
    "postgresql://thought_test:test_password@localhost:5433/thought_test";
  process.env.DB_HOST = process.env.DB_HOST ?? "localhost";
  process.env.DB_PORT = process.env.DB_PORT ?? "5433";
  process.env.DB_NAME = process.env.DB_NAME ?? "thought_test";
  process.env.DB_USER = process.env.DB_USER ?? "thought_test";
...
async function waitForPostgresReady(maxAttempts = 30, intervalMs = 1000): Promise<void> {
  ...
  const dbManager = new DatabaseConnectionManager({
    ...
    database: process.env.DB_NAME ?? "thought_test",
    user: process.env.DB_USER ?? "thought_test",
