...
describe("FrameworkSelector", () => {
  ...
  describe("selectFramework", () => {
    ...
    it("should respect preferredFramework override", () => {
      const problem: Problem = {
        id: "override-test",
        description: "Simple problem that would normally trigger Scientific Method",
        context: "User wants creative approach",
        goals: ["Be creative"],
        constraints: [],
        complexity: "simple",
      };

      // Identify what it would normally select (Scientific Method for simple execution)
      const normalSelection = selector.selectFramework(problem, createContext(problem));
      expect(normalSelection.primaryFramework.id).toBe("scientific-method");

      // Now force Design Thinking
      const options = {
        context: createContext(problem),
        preferredFrameworkId: "design-thinking",
      };

      const forcedSelection = selector.selectFramework(problem, options);

      expect(forcedSelection.primaryFramework.id).toBe("design-thinking");
      expect(forcedSelection.confidence).toBeGreaterThanOrEqual(0.9);
      expect(forcedSelection.reason).toContain("User explicitly preferred");
    });
  });
