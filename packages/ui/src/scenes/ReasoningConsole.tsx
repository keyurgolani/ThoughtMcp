 ...
 * Leverages Thought's parallel reasoning capabilities with support for
    ...
    try {
      if (selectedMode === "parallel") {
        ...
        if ("status" in response) {
          // Async mode started - wait for completion
          // The parallel reasoning display will show progress
        } else {
          // Fallback to sync if server didn't accept async
          const syncResponse = response;
          setResult({ type: "parallel", data: syncResponse });
          setProgress(100);
          completeOperation(operationId, { type: "think_parallel", data: syncResponse });
