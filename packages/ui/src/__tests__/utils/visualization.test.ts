...
describe('getSectorColor', () => {
  ...
  it('should return correct standard colors for each sector', () => {
    expect(getSectorColor('episodic')).toBe('#E8B86D'); // Muted gold/amber
    expect(getSectorColor('semantic')).toBe('#6BA3BE'); // Steel blue
    expect(getSectorColor('procedural')).toBe('#8B7BB5'); // Muted purple
    expect(getSectorColor('emotional')).toBe('#C97B63'); // Terracotta
    expect(getSectorColor('reflective')).toBe('#9CA3AF'); // Cool gray
  });

  it('should return correct high contrast colors for each sector', () => {
    expect(getSectorColor('episodic', true)).toBe('#F5D08A'); // Soft gold
    expect(getSectorColor('semantic', true)).toBe('#7EC8E3'); // Light blue
    expect(getSectorColor('procedural', true)).toBe('#A594C9'); // Light purple
    expect(getSectorColor('emotional', true)).toBe('#E09B84'); // Soft coral
    expect(getSectorColor('reflective', true)).toBe('#D1D5DB'); // Light gray
  });

  it('should return correct light mode colors for each sector', () => {
    expect(getSectorColor('episodic', false, true)).toBe('#D4880F'); // Bold amber/gold
    expect(getSectorColor('semantic', false, true)).toBe('#0077B6'); // Bold blue
    expect(getSectorColor('procedural', false, true)).toBe('#7B2CBF'); // Bold purple
    expect(getSectorColor('emotional', false, true)).toBe('#C44536'); // Bold red-orange
    expect(getSectorColor('reflective', false, true)).toBe('#495057'); // Bold gray
  });
  ...
  it('should default to standard colors when highContrast is false', () => {
    expect(getSectorColor('episodic', false)).toBe('#E8B86D');
  });

  it('should prioritize light mode over high contrast', () => {
    // Light mode takes precedence
    expect(getSectorColor('episodic', true, true)).toBe('#D4880F');
  });
});
