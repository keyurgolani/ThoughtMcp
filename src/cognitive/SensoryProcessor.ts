...
export class SensoryProcessor implements ISensoryProcessor {
  ...
  async process(input: string): Promise<ProcessedInput> {
    ...
    try {
      ...
      const filtered_tokens = this.filterAttention(
        tokens,
        this.attention_threshold
      );
      ...
      const semantic_chunks = this.createSemanticChunks(
        filtered_tokens,
        patterns
      );
  ...
  filterAttention(tokens: Token[], threshold: number): Token[] {
    ...
    if (filtered.length < min_retention) {
      ...
      const sorted = [...tokens].sort((a, b) => {
        const scoreA =
          a.attention_score + (this.isHighImportanceToken(a) ? 0.5 : 0);
        const scoreB =
          b.attention_score + (this.isHighImportanceToken(b) ? 0.5 : 0);
  ...
  computeSalience(tokens: Token[]): SalienceMap {
    const scores = tokens.map((token) =>
      this.computeTokenSalience(token, tokens)
    );

    // Normalize scores
    const max_score = Math.max(...scores);
    const normalized_scores =
      max_score > 0 ? scores.map((s) => s / max_score) : scores;
  ...
  private isHighImportanceToken(token: Token): boolean {
    ...
    return (
      token.context_tags.includes("entity") ||
  ...
  private detectSemanticPatterns(tokens: Token[]): Pattern[] {
    ...
    categories.forEach((cluster_tokens, _category) => {
      if (cluster_tokens.length >= 2) {
        patterns.push({
          ...
          confidence: Math.min(
            0.9,
            cluster_tokens.length / tokens.length + 0.3
          ),
          salience:
            cluster_tokens.reduce((sum, t) => sum + t.semantic_weight, 0) /
            cluster_tokens.length,
        });
      }
    });
    ...
    if (aiTerms.length >= 2) {
      patterns.push({
        ...
        confidence: 0.8,
        salience:
          aiTerms.reduce((sum, t) => sum + t.semantic_weight, 0) /
          aiTerms.length,
      });
    }
  ...
  private detectSyntacticPatterns(tokens: Token[]): Pattern[] {
    ...
    for (let i = 0; i < tokens.length - 1; i++) {
      ...
      // Noun-Verb pattern
      if (
        current.context_tags.includes("noun") &&
        next.context_tags.includes("verb")
      ) {
        patterns.push({
          type: "noun_verb",
      ...
      if (
        current.context_tags.includes("adjective") &&
        next.context_tags.includes("noun")
      ) {
        patterns.push({
          type: "adjective_noun",
          content: [current.text, next.text],
  ...
  private createSemanticChunks(
    ...
    patterns.forEach((pattern) => {
      if (pattern.confidence > 0.5) {
        const chunk_tokens = tokens.filter(
          (token) =>
            pattern.content.includes(token.text) &&
            !used_tokens.has(token.position)
        );
    ...
    if (remaining_tokens.length > 0) {
      chunks.push({
        ...
        importance:
          remaining_tokens.reduce((sum, t) => sum + t.semantic_weight, 0) /
  ...
  private computeSequenceConfidence(sequence: Token[]): number {
    const avg_attention =
      sequence.reduce((sum, t) => sum + t.attention_score, 0) / sequence.length;
    const avg_semantic =
      sequence.reduce((sum, t) => sum + t.semantic_weight, 0) / sequence.length;
  ...
  private computeContextRelevance(word: string, context: string[]): number {
    ...
    const related_count = context.filter((w) =>
      related_words.includes(w)
    ).length;
  ...
  private classifySemanticCategory(word: string): string | null {
    ...
    if (entity_indicators.some((indicator) => word.includes(indicator)))
      return "entity";
    if (action_indicators.some((indicator) => word.includes(indicator)))
      return "action";
    if (property_indicators.some((indicator) => word.includes(indicator)))
      return "property";
