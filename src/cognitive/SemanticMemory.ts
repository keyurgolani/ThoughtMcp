...
import { ComponentStatus, ISemanticMemory } from "../interfaces/cognitive.js";
...
export class SemanticMemory implements ISemanticMemory {
  ...
  async process(input: unknown): Promise<unknown> {
    ...
    const inputObj = input as { concept?: unknown };
    if (typeof input === "object" && input !== null && inputObj?.concept) {
      return this.store(inputObj.concept as Concept);
    }
  ...
  retrieve(
    cue: string,
    threshold: number = this.config.similarity_threshold
  ): Concept[] {
    this.lastActivity = Date.now();
  ...
  getRelated(conceptId: string): Concept[] {
    ...
    for (const relationId of concept.relations) {
      ...
      const otherConceptId =
        relation.from === conceptId ? relation.to : relation.from;
  ...
  applyDecay(): void {
    ...
    for (const [id, concept] of this.concepts) {
      const timeSinceAccess =
        (currentTime - concept.last_accessed) / (1000 * 60 * 60); // hours
  ...
  private removeWeakestRelation(conceptId: string): void {
    ...
    if (weakestRelationId) {
      // Remove from concept
      const updatedRelations = concept.relations.filter(
        (id) => id !== weakestRelationId
      );
      const updatedConcept: Concept = {
        ...concept,
  ...
  private pruneLeastActiveConcepts(): void {
    ...
    const sortedByActivation = conceptEntries.sort(
      (a, b) => a[1].activation - b[1].activation
    );
  ...
  private removeConcept(conceptId: string): void {
    ...
    for (const [id, otherConcept] of this.concepts) {
      const updatedRelations = otherConcept.relations.filter((relationId) => {
        const relation = this.relations.get(relationId);
        return (
          relation && relation.from !== conceptId && relation.to !== conceptId
        );
      });
