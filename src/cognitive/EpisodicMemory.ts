...
import { IEpisodicMemory, ComponentStatus } from '../interfaces/cognitive.js';
...
export class EpisodicMemory implements IEpisodicMemory {
  ...
  async process(input: any): Promise<any> {
    ...
    if (typeof input === 'object' && input.episode) {
      return this.store(input.episode);
    }
   ...
   */
  retrieve(cue: string, threshold: number = this.config.retrieval_threshold): Episode[] {
    this.lastActivity = Date.now();
  ...
  decay(): void {
    ...
    for (const [id, episode] of this.episodes) {
      ...
      const newDecayFactor = episode.decay_factor * Math.exp(-this.config.decay_rate * timeDelta);
  ...
  consolidate(): Episode[] {
    ...
    for (const episode of this.episodes.values()) {
      ...
      if (episode.importance >= this.config.consolidation_threshold && 
  ...
  getEpisodesByContext(contextKey: string, contextValue: string): Episode[] {
    ...
    for (const episode of this.episodes.values()) {
      if (episode.context && (episode.context as any)[contextKey] === contextValue) {
        episodes.push(episode);
      }
    }
  ...
  private updateContextIndex(episodeId: string, episode: Episode): void {
    ...
    if (episode.context.session_id) {
      this.addToIndex(this.contextIndex, `session:${episode.context.session_id}`, episodeId);
    }
    
    // Index by domain
    if (episode.context.domain) {
      this.addToIndex(this.contextIndex, `domain:${episode.context.domain}`, episodeId);
    }
  ...
  private addToIndex<K>(index: Map<K, Set<string>>, key: K, episodeId: string): void {
    if (!index.has(key)) {
  ...
  private computeRetrievalScore(episode: Episode, cue: string): number {
    ...
    // Recency boost
    const hoursSinceCreation = (Date.now() - episode.timestamp) / (1000 * 60 * 60);
    ...
    if (episode.emotional_tags && episode.emotional_tags.length > 0) {
      const emotionalWords = ['happy', 'sad', 'angry', 'fear', 'surprise', 'disgust'];
  ...
  private pruneOldestEpisodes(): void {
    ...
    const sortedByAge = episodeEntries.sort((a, b) => a[1].timestamp - b[1].timestamp);
