...
export class CognitiveMCPServer implements IMCPServer, IToolHandler {
    ...
    this.server = new Server(
      {
        name: 'thought-mcp',
        version: '1.0.0',
        description: 'MCP server implementing human-like cognitive architecture for enhanced AI reasoning'
      },
      {
        capabilities: {
          tools: {}
  ...
  registerTools(): void {
    try {
      ...
      this.server.setRequestHandler(ListToolsRequestSchema, async () => {
        ...
        console.error(`Registered ${tools.length} cognitive tools: ${tools.map(t => t.name).join(', ')}`);
        return { tools };
      });
      ...
      this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
        ...
        if (!Object.keys(TOOL_SCHEMAS).includes(name)) {
          throw new Error(`Unknown tool: ${name}. Available tools: ${Object.keys(TOOL_SCHEMAS).join(', ')}`);
        }
  ...
  async handleRequest(_method: string, _params: any): Promise<any> {
  ...
  private validateThinkArgs(args: any): ThinkArgs {
    if (!args.input || typeof args.input !== 'string') {
      throw new Error('Think tool requires a valid input string');
    }

    if (args.mode && !Object.values(ProcessingMode).includes(args.mode)) {
      throw new Error(`Invalid processing mode: ${args.mode}. Valid modes: ${Object.values(ProcessingMode).join(', ')}`);
    }

    if (args.temperature !== undefined && (typeof args.temperature !== 'number' || args.temperature < 0 || args.temperature > 2)) {
      throw new Error('Temperature must be a number between 0 and 2');
    }

    if (args.max_depth !== undefined && (typeof args.max_depth !== 'number' || args.max_depth < 1 || args.max_depth > 20)) {
      throw new Error('Max depth must be a number between 1 and 20');
    }

    return args as ThinkArgs;
  }

  private validateRememberArgs(args: any): RememberArgs {
    ...
    if (args.importance !== undefined && (typeof args.importance !== 'number' || args.importance < 0 || args.importance > 1)) {
      throw new Error('Importance must be a number between 0 and 1');
    }

    return args as RememberArgs;
  }

  private validateRecallArgs(args: any): RecallArgs {
    if (!args.cue || typeof args.cue !== 'string') {
      throw new Error('Recall tool requires a valid cue string');
    }

    if (args.type && !['episodic', 'semantic', 'both'].includes(args.type)) {
    ...
    if (args.max_results !== undefined && (typeof args.max_results !== 'number' || args.max_results < 1 || args.max_results > 50)) {
      throw new Error('Max results must be a number between 1 and 50');
    }

    return args as RecallArgs;
  }

  private validateAnalyzeReasoningArgs(args: any): AnalyzeReasoningArgs {
    if (!args.reasoning_steps || !Array.isArray(args.reasoning_steps)) {
      throw new Error('Analyze reasoning tool requires an array of reasoning steps');
    }
    ...
    for (let i = 0; i < args.reasoning_steps.length; i++) {
      ...
      if (step.confidence === undefined || typeof step.confidence !== 'number' || step.confidence < 0 || step.confidence > 1) {
        throw new Error(`Reasoning step ${i} requires a confidence number between 0 and 1`);
      }
    }

    return args as AnalyzeReasoningArgs;
  }
  ...
  async handleThink(args: ThinkArgs): Promise<ThoughtResult> {
    ...
    const validatedArgs = this.validateThinkArgs(args);
    const startTime = Date.now();
    
    try {
      console.error(`Processing think request: ${validatedArgs.input.substring(0, 100)}${validatedArgs.input.length > 100 ? '...' : ''}`);
      ...
      console.error(`Think request completed in ${result.metadata.processing_time_ms}ms`);
      return result;
    } catch (error) {
  ...
  async handleAnalyzeReasoning(args: AnalyzeReasoningArgs): Promise<AnalysisResult> {
    // Validate arguments first
    const validatedArgs = this.validateAnalyzeReasoningArgs(args);
    const startTime = Date.now();
    
    try {
      console.error(`Analyzing ${validatedArgs.reasoning_steps.length} reasoning steps`);
      ...
      const result: AnalysisResult = {
        ...
        detected_biases: [],
        suggested_improvements: ['This is a placeholder analysis - actual implementation pending'],
        reasoning_quality: {
      ...
      console.error(`Reasoning analysis completed in ${Date.now() - startTime}ms`);
      return result;
    } catch (error) {
      console.error('Error in handleAnalyzeReasoning:', error);
      throw new Error(`Reasoning analysis failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
