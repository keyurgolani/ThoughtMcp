...
function convertToInternalChain(
  ...
  return {
    ...
    branches: [],
    evidence:
      reasoningChain.evidence?.map((e, idx) => ({
        id: `evidence-${idx}`,
        content: e,
        source: "user_provided",
      })) ?? [],
    assumptions:
      reasoningChain.assumptions?.map((a) => ({
        id: a.id,
        content: a.content,
        explicit: a.explicit,
    ...
    inferences:
      reasoningChain.inferences?.map((i) => ({
        id: i.id,
        content: i.content,
        premises: i.premises,
    ...
    conclusion: reasoningChain.steps?.find((s) => s.type === "conclusion")?.content ?? "",
  };
}
