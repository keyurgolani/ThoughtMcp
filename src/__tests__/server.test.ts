...
describe('CognitiveMCPServer', () => {
  ...
  describe('Tool Handlers', () => {
    it('should handle think requests', async () => {
      ...
      expect(result.content).toContain('What is consciousness?');
    ...
    it('should handle analyze reasoning requests', async () => {
      ...
      expect(result.reasoning_quality.logical_consistency).toBeGreaterThanOrEqual(0);
      expect(result.reasoning_quality.logical_consistency).toBeLessThanOrEqual(1);
    });
  });
  ...
  describe('Input Validation', () => {
    describe('Think Tool Validation', () => {
      it('should reject empty input', async () => {
        await expect(server.handleThink({ input: '' })).rejects.toThrow('Think tool requires a valid input string');
      });
    ...
    describe('Recall Tool Validation', () => {
      it('should reject empty cue', async () => {
        await expect(server.handleRecall({ cue: '' })).rejects.toThrow('Recall tool requires a valid cue string');
      });
    ...
    describe('Analyze Reasoning Tool Validation', () => {
      ...
      it('should reject invalid reasoning step structure', async () => {
        ...
        await expect(server.handleAnalyzeReasoning({ 
          reasoning_steps: [{ type: 'test', content: 'test', confidence: -1 }] 
        })).rejects.toThrow('Reasoning step 0 requires a confidence number between 0 and 1');
      });
  ...
  describe('Error Handling', () => {
    it('should handle processing errors gracefully', async () => {
      ...
      await expect(server.handleThink({ input: 'test' })).rejects.toThrow('Simulated processing error');
