...
describe("SensoryProcessor", () => {
  ...
  describe("Tokenization", () => {
    ...
    it("should assign semantic weights to tokens", () => {
      ...
      expect(theToken?.semantic_weight).toBeLessThan(
        intelligentToken?.semantic_weight ?? 0
      );
      expect(theToken?.semantic_weight).toBeLessThan(
        systemToken?.semantic_weight ?? 0
      );
    });
  ...
  describe("Attention Filtering", () => {
    ...
    it("should compute dynamic threshold based on input complexity", () => {
      ...
      expect(
        complexFiltered.length / complexTokens.length
  ...
  describe("Pattern Detection", () => {
    it("should detect sequential patterns (n-grams)", () => {
      ...
      const mlPattern = bigramPatterns.find(
        (p) => p.content.includes("machine") && p.content.includes("learning")
      );
    ...
    it("should detect semantic patterns", () => {
      ...
      const semanticPatterns = patterns.filter(
        (p) => p.type === "semantic_cluster"
      );
      expect(semanticPatterns.length).toBeGreaterThan(0);

      // Should cluster entities and actions separately
      const entityCluster = semanticPatterns.find(
        (p) => p.content.includes("dog") && p.content.includes("cat")
      );
    ...
    it("should detect syntactic patterns", () => {
      ...
      const adjectiveNounPattern = patterns.find(
        (p) => p.type === "adjective_noun"
      );
  ...
  describe("Salience Computation", () => {
    it("should compute salience scores for all tokens", () => {
      ...
      expect(salienceMap.scores[importantIndex]).toBeGreaterThan(
        salienceMap.scores[theIndex]
      );
    });
  ...
  describe("Full Processing Pipeline", () => {
    it("should process input through complete pipeline", async () => {
      const input =
        "The intelligent cognitive system processes complex information efficiently using advanced algorithms.";
    ...
    it("should handle complex input with multiple patterns", async () => {
      const input =
        "The advanced artificial intelligence system uses machine learning algorithms to process natural language understanding tasks efficiently and accurately.";
      ...
      const semanticClusters = result.patterns.filter(
        (p) => p.type === "semantic_cluster"
      );
      expect(semanticClusters.length).toBeGreaterThan(0);

      // Should have n-gram patterns
      const ngramPatterns = result.patterns.filter((p) =>
        p.type.includes("-gram")
      );
    ...
    it("should handle processing errors gracefully", async () => {
      ...
      await expect(processor.process("test input")).rejects.toThrow(
        "Processing failed"
      );
  ...
  describe("Reset and Status", () => {
    ...
    it("should handle uninitialized processor", async () => {
      ...
      await expect(uninitializedProcessor.process("test")).rejects.toThrow(
        "SensoryProcessor not initialized"
      );
