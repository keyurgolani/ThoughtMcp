...
describe('DeliberativeProcessor', () => {
  ...
  beforeEach(async () => {
    ...
    mockInput = {
      input: 'Analyze the relationship between economic growth and environmental sustainability',
      context: {
  ...
  describe('Reasoning Tree Construction', () => {
    it('should build reasoning tree with correct structure', () => {
      const tree = processor.buildReasoningTree('What causes climate change?', 3);
  ...
  describe('Option Generation and Evaluation', () => {
    it('should generate multiple reasoning options', () => {
      const options = processor['generateDeliberativeOptions']('How can we solve poverty?');
    ...
    it('should evaluate options and rank by confidence', () => {
      ...
      expect(evaluated[0].confidence).toBeGreaterThanOrEqual(evaluated[1].confidence);
    });
  ...
  describe('Reasoning Strategies', () => {
    it('should apply deductive reasoning', () => {
      const options = processor['applyDeductiveReasoning']('All humans are mortal. Socrates is human.');
    ...
    it('should apply inductive reasoning', () => {
      const options = processor['applyInductiveReasoning']('I observed 100 swans and they were all white.');
      
      expect(options.length).toBeGreaterThan(0);
      expect(options[0].content).toContain('general patterns');
      expect(options[0].reasoning_chain).toContain('Identify recurring patterns');
    });

    it('should apply abductive reasoning', () => {
      const options = processor['applyAbductiveReasoning']('The grass is wet this morning.');
      
      expect(options.length).toBeGreaterThan(0);
      expect(options[0].content).toContain('most plausible explanation');
      expect(options[0].reasoning_chain).toContain('Generate possible explanations');
    });

    it('should apply analogical reasoning', () => {
      const options = processor['applyAnalogicalReasoning']('This situation is like a chess game.');
    ...
    it('should apply causal reasoning', () => {
      const options = processor['applyCausalReasoning']('What caused the economic recession?');
  ...
  describe('Deliberative Processing', () => {
    ...
    it('should generate detailed reasoning path', async () => {
      ...
      expect(result.reasoning_path.some(step => step.type === ReasoningType.LOGICAL_INFERENCE)).toBe(true);
    });
    ...
    it('should handle complex analytical tasks', async () => {
      const complexInput = {
        ...mockInput,
        input: 'Evaluate the pros and cons of implementing universal basic income, considering economic, social, and political factors'
      };
  ...
  describe('Emotional Context Assessment', () => {
    it('should assess emotional context for deliberative thinking', async () => {
      ...
      expect(result.emotional_context.specific_emotions.has('analytical')).toBe(true);
    });
  ...
  describe('Response Generation', () => {
    it('should generate comprehensive responses', async () => {
      ...
      expect(result.content).toContain('systematic analysis');
  ...
  describe('Evidence Evaluation', () => {
    it('should evaluate evidence quality', () => {
      const goodEvidence = ['empirical data', 'peer reviewed studies', 'statistical analysis'];
