...
describe("EpisodicMemory", () => {
  ...
  describe("retrieval", () => {
    ...
    it("should retrieve relevant episodes based on content cue", () => {
      ...
      expect((results[0].content as { text: string }).text).toContain(
        "machine learning"
      );
    });
  ...
  describe("context-based retrieval", () => {
    ...
    it("should retrieve episodes by context domain", () => {
      ...
      const cookingEpisodes = episodicMemory.getEpisodesByContext(
        "domain",
        "cooking"
      );
      ...
      expect((aiEpisodes[0].content as { text: string }).text).toContain(
        "AI research"
      );
      expect((cookingEpisodes[0].content as { text: string }).text).toContain(
        "Cooking recipe"
      );
    });

    it("should retrieve episodes by session ID", () => {
      const session1Episodes = episodicMemory.getEpisodesByContext(
        "session_id",
        "session1"
      );
      const session2Episodes = episodicMemory.getEpisodesByContext(
        "session_id",
        "session2"
      );
  ...
  describe("temporal operations", () => {
    it("should retrieve episodes by time range", () => {
      ...
      expect((recentEpisodes[0].content as { text: string }).text).toBe(
        "Recent memory"
      );
    });
  });
  ...
  describe("consolidation", () => {
    it("should identify episodes ready for consolidation", () => {
      ...
      expect(
        (consolidationCandidates[0].content as { text: string }).text
      ).toBe("High importance memory");
    });
  });
