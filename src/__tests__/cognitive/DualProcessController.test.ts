...
describe('DualProcessController', () => {
  ...
  describe('Processing Decision Making', () => {
    ...
    it('should decide to use both systems for complex inputs', async () => {
      const complexInput = {
        ...mockInput,
        input: 'Analyze the complex interrelationships between economic policy, environmental sustainability, and social equity in the context of global climate change',
    ...
    it('should consider urgency in decision making', async () => {
      ...
      expect(decision.reasoning.toLowerCase()).toContain('urgency') || expect(decision.use_system2).toBe(true);
    });
    ...
    it('should detect uncertainty indicators', async () => {
      ...
      expect(decision.reasoning.toLowerCase()).toContain('uncertainty') || decision.use_system2;
    });
  });
  ...
  describe('Complexity Assessment', () => {
    ...
    it('should assess high complexity for complex inputs', () => {
      const complexText = 'Analyze the systematic relationships between multiple variables while considering the implications of various conditional scenarios and their potential consequences';
    ...
    it('should detect complex question structures', () => {
      const isComplex = controller['isComplexQuestion']('What happens if we do this and how does it affect that when considering these conditions?');
  ...
  describe('Conflict Resolution', () => {
    it('should resolve conflicts between systems', async () => {
      ...
      const resolution = controller['resolveConflict'](mockSystem1Result, mockSystem2Result, mockInput);
  ...
  describe('Result Blending', () => {
    it('should blend compatible results', async () => {
      ...
      const blended = controller['blendResults'](mockSystem1Result, mockSystem2Result, resolution);
      ...
      expect(blended.metadata.components_used).toContain('DualProcessController');
    });
  });
  ...
  describe('Metadata Enhancement', () => {
    it('should enhance metadata with dual-process information', async () => {
      ...
      expect(result.metadata.dual_process_info.dual_process_decision).toBeDefined();
      expect(result.metadata.dual_process_info.system1_confidence).toBeDefined();
      expect(result.metadata.dual_process_info.total_processing_time).toBeDefined();
    });
  });
