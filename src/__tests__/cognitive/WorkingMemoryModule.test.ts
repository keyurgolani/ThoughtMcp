...
describe("WorkingMemoryModule", () => {
  ...
  describe("Initialization", () => {
    ...
    it("should set capacity from configuration", async () => {
      ...
      await workingMemory.initialize(config as unknown as Record<string, unknown>);
    ...
    it("should handle initialization errors", async () => {
      ...
      await expect(workingMemory.initialize(invalidConfig)).rejects.toThrow();
    });
  });
  ...
  describe("Buffer System", () => {
    ...
    it("should route text content to phonological buffer", () => {
      ...
      expect(
        state.buffer_states.get(BufferType.PHONOLOGICAL)?.chunks
      ).toHaveLength(1);
    });
    ...
    it("should route spatial content to visuospatial buffer", () => {
      ...
      expect(
        state.buffer_states.get(BufferType.VISUOSPATIAL)?.chunks
      ).toHaveLength(1);
    });
    ...
    it("should route complex content to episodic buffer", () => {
      ...
      expect(state.buffer_states.get(BufferType.EPISODIC)?.chunks).toHaveLength(
        1
      );
    });
  });
  ...
  describe("Information Chunking", () => {
    ...
    it("should chunk sequential arrays", async () => {
      ...
      const hasArrayContent = state.active_chunks.some(
        (chunk) =>
